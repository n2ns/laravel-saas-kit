    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"
        ...
        <div class="flex flex-col md:flex-row gap-0 md:gap-10">
            ...
            <!-- ============================================================
                 Sidebar
                 ============================================================ -->
            <aside class="w-full md:w-60 py-4 md:py-8 md:sticky md:top-20 md:h-[calc(100vh-80px)] flex flex-col border-b md:border-b-0 md:border-r border-white/[0.06]">
                ...
                <nav role="tablist" aria-label="{{ __('Dashboard') }}"
                    ...
                    <button id="tab-{{ $item['tab'] }}"
                            ...
                            :class="activeTab === '{{ $item['tab'] }}'
                                ? 'bg-white/[0.06] text-white border-l-2 border-primary-500'
                    ...
                    <div class="mt-auto pt-4 border-t border-white/[0.06]">
                        <form action="{{ route('logout') }}" method="POST">
                            ...
                            <button type="submit"
                                    class="group flex items-center gap-2.5 px-4 py-2.5 w-full rounded-xl text-sm font-normal text-slate-500 hover:text-red-400 hover:bg-white/[0.04] transition-all text-left cursor-pointer">
            ...
            <main class="flex-1 py-10 overflow-hidden min-w-0">
                ...
                <div id="panel-overview" role="tabpanel" aria-labelledby="tab-overview" x-show="activeTab === 'overview'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-8">
                    <div>
                        <h2 class="text-2xl font-bold text-white tracking-tight mb-1">{{ __('Welcome back, :name!', ['name' => $user['name']]) }}</h2>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div class="ui-card p-6 rounded-2xl flex items-center gap-4">
                            ...
                            <div>
                                <div class="text-2xl font-bold text-white mb-0.5">{{ $subscriptions->count() }}</div>
                        ...
                        <div class="ui-card p-6 rounded-2xl flex items-center gap-4">
                            ...
                            <div>
                                <div class="text-2xl font-bold text-white mb-0.5">{{ $subscriptions->filter(fn($s) => $s->plan && in_array($s->plan->tier, ['pro', 'enterprise']))->count() }}</div>
                        ...
                        <div class="ui-card p-6 rounded-2xl flex items-center gap-4">
                            ...
                            <div>
                                <div class="text-2xl font-bold text-white mb-0.5">{{ $orders->count() }}</div>
                    ...
                    <div class="ui-card p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-5">
                        <div class="min-w-0">
                            <h3 class="text-sm font-bold text-white mb-1">{{ __('Subscription Snapshot') }}</h3>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <a href="{{ localized_route('products.index') }}"
                               class="ui-card ui-card-lift p-5 rounded-2xl flex flex-col items-center gap-3 text-slate-300 hover:text-white no-underline text-sm font-normal cursor-pointer text-center">
                                <i data-lucide="shopping-bag" class="w-5 h-5 text-slate-400"></i>
                                <span>{{ __('Browse Products') }}</span>
                            </a>
                            <button @click="go('subscriptions')"
                                    class="ui-card ui-card-lift p-5 rounded-2xl flex flex-col items-center gap-3 text-slate-300 hover:text-white text-sm font-normal cursor-pointer text-center border-0">
                                <i data-lucide="layers" class="w-5 h-5 text-slate-400"></i>
                                <span>{{ __('Manage Subscriptions') }}</span>
                            </button>
                            <a href="{{ localized_route('support') }}"
                               class="ui-card ui-card-lift p-5 rounded-2xl flex flex-col items-center gap-3 text-slate-300 hover:text-white no-underline text-sm font-normal cursor-pointer text-center">
                                <i data-lucide="life-buoy" class="w-5 h-5 text-slate-400"></i>
                ...
                <div id="panel-orders" role="tabpanel" aria-labelledby="tab-orders" x-show="activeTab === 'orders'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6">
                    <div>
                        <h2 class="text-2xl font-bold text-white tracking-tight mb-1">{{ __('Order History') }}</h2>
                    ...
                    <div class="p-14 text-center rounded-2xl ui-card border-dashed">
                        ...
                        <h3 class="text-base font-bold text-white mb-2">{{ __('No orders found') }}</h3>
                        <p class="text-sm text-slate-500">{{ __('Your order history will appear here after purchase.') }}</p>
                    </div>
                    @else
                    <div class="overflow-x-auto rounded-2xl border border-white/[0.08] bg-white/[0.02] scrollbar-thin">
                        <table class="w-full text-left text-sm border-collapse">
                            ...
                            <tbody class="divide-y divide-white/[0.04]">
                                ...
                                <tr class="hover:bg-white/[0.03] transition-colors">
                                    ...
                                    <td class="px-5 py-4 text-slate-300 text-sm">{{ $order->plan->name ?? 'N/A' }}</td>
                                    <td class="px-5 py-4 font-bold text-white text-sm">{{ strtoupper($order->currency) }} {{ number_format($order->total, 2) }}</td>
                                    <td class="px-5 py-4">
                ...
                <div id="panel-subscriptions" role="tabpanel" aria-labelledby="tab-subscriptions" x-show="activeTab === 'subscriptions'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6">
                    <div>
                        <h2 class="text-2xl font-bold text-white tracking-tight mb-1">{{ __('Your Subscriptions') }}</h2>
                    ...
                    <div class="p-14 text-center rounded-2xl ui-card border-dashed">
                        ...
                        <h3 class="text-base font-bold text-white mb-2">{{ __('No active subscriptions') }}</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="ui-card ui-card-lift p-6 rounded-2xl">
                            <div class="flex items-start gap-4 mb-5">
                                @if($product?->image)
                                <img src="{{ asset($product->image) }}" alt="{{ $product->name }}" class="w-14 h-14 rounded-xl object-cover border border-white/[0.08] flex-shrink-0">
                                @else
                                ...
                                <div class="flex-1 min-w-0">
                                    <h3 class="text-sm font-bold text-white truncate mb-0.5">{{ $product?->name ?? $subscription->type }}</h3>
                            ...
                            <div class="space-y-2.5 py-4 border-y border-white/[0.06] mb-5">
                                <div class="flex justify-between text-sm">
                                    <span class="text-slate-500">{{ __('Billing') }}</span>
                                    <span class="text-slate-300">{{ ucfirst($plan?->billing_cycle ?? 'monthly') }}</span>
                                </div>
                                @if($plan?->price)
                                <div class="flex justify-between text-sm">
                                    <span class="text-slate-500">{{ __('Price') }}</span>
                                    <span class="text-white font-bold">{{ strtoupper($plan->currency ?? 'USD') }} {{ number_format($plan->price, 2) }}</span>
                                </div>
                                @endif
                            ...
                            <div class="flex gap-2.5">
                                ...
                                <a href="{{ localized_route('checkout.portal', ['product' => $product->code]) }}"
                                   class="flex-1 text-center py-2 rounded-xl border border-white/[0.1] text-xs font-semibold text-slate-400 hover:text-white hover:bg-white/5 transition-all">
                                    {{ __('Manage') }}
                                </a>
                ...
                <div id="panel-settings" role="tabpanel" aria-labelledby="tab-settings" x-show="activeTab === 'settings'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-10">
                    <div>
                        <h2 class="text-2xl font-bold text-white tracking-tight mb-1">{{ __('Account Settings') }}</h2>
                    ...
                    <div class="space-y-8">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center justify-between p-4 rounded-xl ui-card">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @endif
                                        <span class="text-sm font-semibold text-slate-300">{{ ucfirst($account->provider) }}</span>
                                    </div>
                        ...
                        <!-- Danger Zone -->
                        <section class="space-y-4 pt-8 border-t border-white/[0.06]">
