    ...
    <article class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 pt-4 relative z-10">
        ...
        <header class="text-left mb-16 animate-fade-in border-l-2 border-primary-500 pl-8">
            ...
            <div class="flex items-center gap-2 text-xs text-slate-500 mb-6">
                <a href="{{ localized_route('catalog.show', ['slug' => $productCode]) }}" class="hover:text-slate-700 transition-colors">{{ $productName }}</a>
                <i data-lucide="chevron-right" class="w-3 h-3"></i>
                <a href="{{ localized_route('catalog.guides.index', ['productCode' => $productCode]) }}" class="hover:text-slate-700 transition-colors">Guides</a>
                <i data-lucide="chevron-right" class="w-3 h-3"></i>
                <span class="text-slate-600 truncate max-w-xs">{{ $guide->getTranslation('title', $contentLocale) }}</span>
            </div>
            ...
            <div class="flex items-center justify-start gap-4 mb-6">
                ...
                <span class="text-slate-600 font-normal text-xs">{{ $guide->getReadingTime($contentLocale) }} min read</span>
            </div>

            <h1 class="text-2xl md:text-5xl font-bold mb-10 text-slate-950 tracking-tight leading-tight">
                {{ $guide->getTranslation('title', $contentLocale) }}
            </h1>

            @if($guide->getTranslation('excerpt', $contentLocale))
                <p class="text-base text-slate-600 mb-8 leading-relaxed font-normal italic">
        ...
        <div class="mt-16 p-8 rounded-2xl bg-primary-500/5 border border-primary-500/20 text-center">
            <p class="text-slate-950 font-semibold mb-2">Try {{ $productName }} for free</p>
            <p class="text-slate-600 text-sm mb-6">Autofill forms in seconds — no manual typing needed.</p>
        ...
        <footer class="mt-12 pt-10 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-8">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-slate-950 font-semibold">{{ $guide->author->name ?? 'SaaS Starter' }}</p>
                    <p class="text-slate-600 text-sm">{{ $productName }} Team</p>
                </div>
            </div>

            <a href="{{ localized_route('catalog.guides.index', ['productCode' => $productCode]) }}"
               class="px-6 py-3 bg-white hover:bg-orange-100/80 text-slate-950 rounded-full border border-black/10 transition-all flex items-center gap-2">
