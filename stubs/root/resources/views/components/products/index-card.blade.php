...
<div x-show="!category || category === '{{ $itemCategory }}'"
    ...
    <a href="{{ $link }}" @if($isExternal) target="_blank" rel="noopener noreferrer" @endif class="block h-[180px] relative overflow-hidden bg-bg-surface flex-shrink-0">
        ...
        />
        <div class="absolute inset-0 bg-gradient-to-t from-white/70 to-transparent opacity-60"></div>
    </a>
    ...
    <div class="p-5 flex flex-col flex-grow">
        ...
        <!-- Title -->
        <h3 class="text-[15px] font-bold text-slate-950 mb-2 tracking-tight">
            <a href="{{ $link }}" class="hover:text-[#a34f1f] transition-colors">{{ $title }}</a>
        </h3>

        <!-- Description -->
        <p class="text-sm text-slate-600 mb-5 flex-grow leading-relaxed line-clamp-2">
            {{ $description }}
        </p>

        <!-- Actions -->
        <div class="flex justify-between items-center gap-3 mt-auto pt-4 border-t border-black/[0.08]">
            @if($link !== '#')
                <a href="{{ $link }}" @if($isExternal) target="_blank" rel="noopener noreferrer" @endif
                   class="text-xs font-semibold text-slate-600 hover:text-[#a34f1f] transition-colors flex items-center gap-1.5 group/btn">
            ...
            @if($isProduct && $pricingLink)
                <a href="{{ $pricingLink }}"
                   class="ui-badge-warning px-3 py-1.5 rounded-full hover:bg-[#a34f1f] hover:text-white transition-all text-xs font-bold whitespace-nowrap">
                    {{ __('products.pricing.title') }}
                </a>
            @elseif($githubUrl)
                <a href="{{ $githubUrl }}" target="_blank" rel="noopener noreferrer"
                   class="w-8 h-8 rounded-lg bg-white border border-black/10 flex items-center justify-center text-slate-600 hover:bg-orange-50 hover:text-slate-950 transition-all duration-200 group/gh"
