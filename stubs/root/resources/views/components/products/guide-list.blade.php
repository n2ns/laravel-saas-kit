...
@if($guideCount > 0)
<section {{ $attributes->merge(['class' => 'mt-12 border-t border-black/[0.08] pt-8']) }}>
    <div class="flex flex-wrap items-end justify-between gap-4 mb-5">
        <div>
            <p class="section-label mb-2">{{ __('products.guides_label') }}</p>
            <h2 class="text-xl md:text-2xl font-semibold text-slate-950 tracking-tight">
    ...
    <div class="divide-y divide-white/[0.06] border-y border-black/[0.08]">
        @foreach($guides as $guide)
            ...
            <a href="{{ $guideUrl }}" class="group grid gap-3 py-5 md:grid-cols-[1fr_auto] md:items-center">
                <div class="min-w-0">
                    ...
                    <h3 class="text-base md:text-lg font-semibold text-slate-800 group-hover:text-[var(--brand-primary)] transition-colors line-clamp-2">
                        {{ $guideTitle }}
                    </h3>
                ...
                <span class="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 group-hover:text-[var(--brand-primary)] transition-colors md:justify-self-end">
