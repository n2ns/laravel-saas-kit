...
<div class="relative flex flex-col items-center justify-center min-h-[60vh] pt-10 pb-16 bg-bg-body type-{{ $product->product_type }} product-{{ $product->code }} overflow-hidden">
    ...
    <div class="container mx-auto px-4 relative z-10 pt-4 text-center">
        ...
        <!-- Title -->
        <h1 class="text-2xl md:text-5xl font-bold text-slate-950 mb-4 tracking-tight leading-tight font-bold">
            {{ __('products.subscription_unavailable') }}
        </h1>

        <!-- Pause Reason Message -->
        <p class="text-base text-slate-600 max-w-xl mx-auto mb-8">
            {{ __('products.pause_reasons.' . $product->pause_reason) }}
        </p>

        <!-- Product Name Badge -->
        <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-black/10 mb-8">
            <span class="text-sm text-slate-700">{{ $product->getLocalized('name') }}</span>
        </div>

        <!-- CTA -->
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href="{{ $product->publicUrl() }}" 
               class="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white/10 hover:bg-white/20 text-slate-950 font-semibold transition-all">
